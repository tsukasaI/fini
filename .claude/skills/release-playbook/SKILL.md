...
| VS Code extension | `editors/vscode/package.json`, tag `vscode-v*` | **decoupled from the CLI.** Own version (currently `0.2.0`, independent of CLI's `0.3.0`) and own tag prefix/workflow (`release-vscode.yaml`). Do NOT bump per CLI release — only bump+tag when the extension itself changed. **Exception:** a release that changes the `--stdin` contract (or `--version` output shape) must ship a matching extension release that updates `COMPATIBLE_CLI_MIN`/`COMPATIBLE_CLI_MAX_EXCLUSIVE` in `editors/vscode/src/extension.ts` (tsukasaI/fini#43) | separate manual release, not part of this checklist unless the extension changed |
